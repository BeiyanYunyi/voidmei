...
def recording_smoke(package, timeout, renderer="OPENGL", hud=True, check_ui=False, compatible_hud=False, graceful_exit=False, display_scale=1, jet=False, wep=False, wep_dropout=False, tray_recovery=False, tray_background=False, hud_renderer=None, poll_interval_ms=100, delayed_sample=False, hud_scene=False):
        ...
        def do_GET(self):
            ...
            if flying.is_set():
                if self.path == "/state":
                    ...
                    if hud_scene:
                        data.update({"gear, %": 0, "flaps, %": 25, "airbrake, %": 100, "elevator, %": 67, "aileron, %": -50, "mixture 1, %": 60, "thrust 1, kgs": 500})
                    if wep:
                        data["throttle 1, %"] = 110
    ...
    if hud_scene:
        ...
        for name, fields in (("messages", ["event"]), ("alerts", ["advisory"]),
                             ...
                             ("engine", ["rpm", "water_temperature", "mixture", "heat_budget", "thrust_power", "propulsive_efficiency"])):
    ...
    for index, (sample, engine) in enumerate(zip(flight, engines)):
        ...
        if hud_scene and not jet and (float(engine["thrust_kgf"]) != 500 or
                abs(float(sample["thrust_power_kw"]) - 354.1290277778) > .001):
            raise RuntimeError("HUD propulsion inputs or recorded thrust power changed at sample %d" % index)
