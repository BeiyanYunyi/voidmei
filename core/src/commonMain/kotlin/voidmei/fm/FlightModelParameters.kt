...
data class FlightModelParameters(
    ...
    val radiatorDrag: List<RadiatorDrag> = emptyList(),
    val aerodynamicPartSourcePaths: Set<String> = aerodynamicParts.map { it.sourcePath }.toSet(),
) {
...
object FlightModelExtractor {
    fun extract(document: BlkBlock, fuel: FuelModification? = null): FlightModelParameters {
        ...
        val radiatorDrag = RadiatorDragExtractor.extract(document)
        issues += radiatorDrag.issues
        ...
        return FlightModelParameters(emptyMass,
            ...
            controlPowerLoss = powerLoss, inertia = inertia.inertia, aerodynamicParts = aerodynamicParts.parts, aerodynamicGeometry = geometry.wings, radiatorDrag = radiatorDrag.fields, aerodynamicPartSourcePaths = aerodynamicParts.sourcePaths)
    }
}
