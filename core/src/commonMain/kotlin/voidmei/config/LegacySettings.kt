...
object LegacySettingsReader {
    ...
    fun read(text: String): LegacySettings {
        ...
        return LegacySettings(interval, flag("crosshairSwitch"), flag("enableVoiceWarn"), volume, choices,
            ...
            }, showCrosshair, crosshairSize, engineControlStyle = controlStyle ?: controlRowSize?.let(::LegacyEngineControlStyle), powerTextSizes = powerSizes ?: powerRowSize?.let(ReadingTextSizes::fromLegacyOffset), enginePanelFonts = (engineRowFont?.let { mapOf("engineInfoSwitch" to it) }.orEmpty() + engineFonts), enginePanelPositions = enginePositions, enginePanelVisibility = enginePanelKeys.mapNotNull { key -> flag(key)?.let { key to it } }.toMap(), engineAircraftFuel = flag("disableEngineInfoLFuel")?.not(), engineReadingColumns = engineColumns, flightReadingColumns = flightColumns, flightLabelFont = flightPanelFont ?: flightLabelFont, flightTextSizes = flightTextSizes ?: flightRowSize?.let(ReadingTextSizes::fromLegacyOffset), attitudeSize = attitudeSize, hudRegionBorders = borderKeys.mapNotNull { (key, content) -> flag(key)?.let { content to it } }.toMap(), hudPositions = positions, hudRegionVisibility = regionSwitches.mapNotNull { (key, content) -> flag(key)?.let { content to it } }.toMap(), hudEngineFieldChoices = engineChoices, softwareRendering = flag("gpuCompatibilityMode"), recordingPerformanceNotifications = flag("enableAltInformation"), connectionNotifications = flag("enableStatusBar"), pendingCrosshairImage = if (!vectorCrosshair)
