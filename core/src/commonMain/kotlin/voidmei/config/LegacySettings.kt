...
data class LegacySettings(val intervalMs: Long?, val hudEnabled: Boolean?, val voiceEnabled: Boolean?, val voiceVolume: Int? = null, val alertVoices: Map<String, VoiceChoice> = emptyMap(),
    ...
    val modelWindows: Boolean? = null, val importModelWindows: Boolean = false, val showImportedModelWindows: Boolean = false,
    ...
    fun applyTo(current: AppSettings) = current.copy(
        ...
        modelWindowEnabled = if (importModelWindows && modelWindows != null) modelWindows && showImportedModelWindows else current.modelWindowEnabled,
        modelJetWindowEnabled = if (importModelWindows && modelWindows != null) modelWindows else current.modelJetWindowEnabled,
        modelJetWindowAutoClose = if (importModelWindows && modelWindows == true) true else current.modelJetWindowAutoClose,
        modelWindowHotkey = if (importModelHotkey && !modelHotkey.isNullOrEmpty()) modelHotkey else current.modelWindowHotkey,
        modelWindowHotkeyEnabled = if ((importModelWindows && modelWindows == false) || (importModelHotkey && modelHotkey == "")) false else current.modelWindowHotkeyEnabled,
        ...
        return LegacySettings(interval, flag("crosshairSwitch"), flag("enableVoiceWarn"), volume, choices,
            ...
            mechanicalVisibility("showHUDAirbrake"), aoaPercent("miniHUDaoaBarWarningRatio"), aoaPercent("miniHUDaoaWarningRatio"), mechanicalVisibility("enableFlapAngleBar"), flag("attitudeIndicatorInertialMode"), flag("autoStartGameMode"), buildMap {
                ...
                LegacyCrosshairImage(crosshairName!!, flag("displayCrosshair"),
                    targets["crosshairScale"]?.second?.toIntOrNull()?.times(2)?.takeIf { it in 24..400 }) else null, textFont = textFont, numberFont = globalNumberFont, httpPort = httpPort, hudAltitudeMode = flag("alwaysShowRadarAltitude")?.let { if (it) HudAltitudeMode.ALWAYS_RADAR else HudAltitudeMode.LOW_RADAR }, hudNumberFont = numberFont, hudReadingColors = readingColors, modelWindows = flag("enableFMPrint"), modelHotkey = modelHotkey, hudAttitudeRefreshMs = attitudeRefresh, hudAttitudeNorthPointer = flag("attitudeIndicatorDisplayDirection"), hudAttitudeAoaLimits = flag("attitudeIndicatorDisplayAoALimits")).also {
            require(it.hasChanges || it.modelWindows != null || it.modelHotkey != null || it.modelSectionChoices.isNotEmpty() || it.engineControlStyle != null || it.powerTextSizes != null || it.enginePanelFonts.isNotEmpty() || it.enginePanelPositions.isNotEmpty() || it.enginePanelVisibility.isNotEmpty() || it.engineReadingColumns != null || it.attitudeSize != null || it.hudRegionBorders.isNotEmpty() || it.flightTextSizes != null || it.flightLabelFont != null || it.flightReadingColumns != null || it.hudPositions.isNotEmpty() || it.hudRegionVisibility.isNotEmpty() || it.unmigrated.isNotEmpty()) { "未找到可迁移的设置" }
        }.let { settings ->
