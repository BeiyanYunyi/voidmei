...
enum class LegacyModelCategory(val target: String, val label: String, val section: ModelDetailSection, val note: String) {
    ...
    FLAPS("showFlapLimits", "襟翼限速", ModelDetailSection.FLAPS, "新版分类还包含当前可用襟翼开度和起落架限速，不等于旧数据点表。"),
    CONTROLS("showControlEffectiveness", "三舵效能", ModelDetailSection.CONTROLS, "当前显示三舵衰减速度；旧锁舵因数尚未补齐。")
}
