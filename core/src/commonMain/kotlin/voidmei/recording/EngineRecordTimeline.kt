...
object EngineRecordTimeline {
    fun analyze(flightText: String, engineText: String, engineIndex: Int, checkActive: () -> Unit = {}): FlightRecordAnalysis =
        read(flightText, engineText, engineIndex, checkActive).analysis

    /** Retain the validated full-resolution join for subsequent window analysis. */
    fun read(flightText: String, engineText: String, engineIndex: Int, checkActive: () -> Unit = {}): EngineTimelineRecord {
        FlightRecordReader.summarize(flightText, checkActive = checkActive)
        require(EngineRecordReader.summarize(engineText, checkActive).any { it.index == engineIndex }) { "记录不包含所选发动机" }
        val flightRows = FlightRecordReader.rows(flightText.removePrefix("\uFEFF"), checkActive).iterator()
        val engineRows = FlightRecordReader.rows(engineText.removePrefix("\uFEFF"), checkActive).iterator()
        ...
        val analysis = FlightRecordPlots.analyze(text, fields = metrics, checkActive = checkActive).copy(
