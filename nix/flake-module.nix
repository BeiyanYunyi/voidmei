...
in {
  perSystem = { config, pkgs, ... }:
    let
      # AWT's native GTK file chooser needs compiled schemas at runtime.
      gtkSchemaPath = pkgs.lib.concatMapStringsSep ":"
        (pkg: "${pkg}/share/gsettings-schemas/${pkg.name}")
        [ pkgs.gtk3 pkgs.gsettings-desktop-schemas ];
    ...
    in {
      packages = {
        default = config.packages.kotlin-offline;
        legacy-java = config.packages.voidmei;
        kotlin-offline = pkgs.callPackage ./kotlin-package.nix {};
        ...
        voidmei = pkgs.stdenvNoCC.mkDerivation {
          ...
          installPhase = ''
            ...
            cp ${./launch.sh} "$out/share/voidmei/launch.sh"
      ...
      apps = {
        kotlin-offline = {
          type = "app";
          program = "${config.packages.kotlin-offline}/bin/voidmei-kotlin";
          meta.description = "Launch the standalone Kotlin application built with pinned dependencies";
        };
        kotlin = {
          type = "app";
          program = "${config.packages.kotlin-launcher}/bin/voidmei-kotlin";
          meta.description = "Build and launch Kotlin development application from a checkout";
        };
        default = config.apps.kotlin-offline;
        legacy-java = {
          type = "app";
          program = "${config.packages.legacy-java}/bin/voidmei";
      ...
      devShells = {
        ...
        default = config.devShells.kotlin;
        legacy-java = pkgs.mkShell {
          inputsFrom = [ config.packages.voidmei ];
          LD_LIBRARY_PATH = config.packages.voidmei.libraryPath;
        };
      };
    };
}
