...
当前初始画布为 1280×720 dp，以现有 HUD 开关和发动机选择建立初始区域。设置中可用滑块调整位置、尺寸、背景／内容不透明度，独立预览沿用相同渲染。第一阶段尚未实现区域增删界面，后续进度见下文。拖拽编辑、UI 修改画布尺寸、锚点、显示器选择与自动覆盖全屏仍待实现，不应称为完整布局编辑器或全屏性能验收。
