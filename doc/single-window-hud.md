...
当前初始画布为 1280×720 dp，以现有 HUD 开关和发动机选择建立初始区域。设置中可用滑块调整位置、尺寸、背景／内容不透明度，独立预览沿用相同渲染。**尚未实现**拖拽编辑、UI 中添加／移除区域或修改画布尺寸、锚点、显示器选择与自动覆盖全屏；配置数据支持重复类型的多个实例。当前不应称为完整布局编辑器或全屏性能验收。
