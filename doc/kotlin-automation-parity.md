...
| 录制状态反馈 | `Controller` 使用通知提示开始录制及日志保存路径。 | 主窗口显示录制状态、路径与错误；录制启动/写入失败复用托盘恢复主窗口流程。开始录制和两份 CSV 正常关闭后的保存成功状态接入托盘通知。 | 原生通知的实际显示受桌面与托盘存续影响，跨平台窗口恢复与通知效果待验证。 |
| 采样结果提示 | enableAltInformation 在 FlightAnalyzer 中控制高度档进展、滚转率与平滑过载观察通知；不是固定周期提示。FlightLog 旧路径以高度变化门槛触发分析。 | 新增默认关闭的 recordingPerformanceNotifications；每个成功写入的录制样本参与观察，100 m 高度档和 10 km/h 速度档；支持旧开关导入，换机/分段重置。 | 原生通知呈现待实机验证；此观察不是完整飞行包线或真实极限，旧分析图表导出仍需单独核对。 |
| 连接状态提示 | `enableStatusBar` 控制 Controller 创建 StatusBar，显示等待服务、等待进入游戏及进入飞行的状态，随后释放窗口。 | 新增默认关闭的 connectionNotifications，导入旧开关；通过托盘提示等待飞行、已收到飞行数据、断线重试。按连接阶段去重，关闭或托盘缺失期间的事件不补发。 | 当前使用托盘通知，未复刻旧独立状态浮窗；各桌面实际气泡效果及无托盘的独立提示仍待验证/迁移。 |
