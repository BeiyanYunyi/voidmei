...
## 离线目录与比较机型持久化（2026-09-12）

新增 `offlineModels` 设置，保存独立离线目录、当前中央机型编号和比较基准中央机型编号。已应用的输入、目录列表选择和“设为比较基准”立即进入应用设置；输入草稿及非法编号不会写入。保存离线选择时固定当前使用的目录，不改变实时 `fmDataRoot`，也不自动打开离线窗口。

重新打开窗口从本地文件加载两个机型，恢复基准的文件来源和新快照时间；当前窗口内仍保持已捕获的基准快照，重新加载当前模型不改变基准。切换目录丢弃旧快照；基准缺失、格式错误或计算失败显示独立状态，可重新加载或清除。新增清除当前离线机型入口。仅保存编号，不持久化快照、燃油修正及功率／推力曲线输入，界面明确告知此范围。

JVM 598、JS 595、桌面单元 153、相关 GUI 4 项通过，0 失败／错误／跳过，日志 `/tmp/voidmei-offline-preferences-tests.log`。覆盖 JSON 往返、兼容缺省设置、非法路径与类型、与实时目录及 HUD 重置隔离，以及窗口内容重建后读取修改后的基准文件、中央编号与 FM 文件名不同、切换目录拒绝旧比较、缺失文件重试、清除与重新选择。未重新打包，未执行完整 GUI、真实游戏或跨平台验证。

旧 `selectedFM0/1` 来自 `fm` 文件列表，而当前加载入口需要中央机型编号并沿 `fmFile` 解析；直接复制可能加载错误模型，因此仍在未迁移清单中，后续需增加有来源的解析与选择。旧 `powerCurveSpeed` 与 `powerCurveWep` 也仍待迁移，本阶段不调整 25 条记录／20 个不同标识的清点。

## 旧模型窗口总开关可选迁移（2026-09-12）

新增 `enableFMPrint` 行的可选迁移预览，支持 switch／switch-inv，默认不选择。旧值启用时配置喷气推力窗口联动及无模型热键时的 10 秒关闭策略，默认将模型浮窗保持隐藏，用户可另选“导入后立即显示模型浮窗”。保留现有模型监听开关，不自动注册热键；旧按键仍单独选择迁移。

旧值关闭时关闭模型浮窗、推力窗口联动和模型热键监听，保留按键、位置、置顶、延迟策略及模型分类偏好；即使同时选择迁移非零按键，也不会重新开启监听。未指定的旧开关保持当前设置。预览明确说明启用后的隐藏／显示结果与现代延迟策略的有效输入、TAS、任一发动机油门适配，不宣称复刻旧模糊字段匹配或平滑速度。

共享 JVM 596、JS 593、桌面单元 153、相关 GUI 55 项通过，无失败、错误或跳过，日志 `/tmp/voidmei-model-window-import-tests.log`。覆盖选择前后、默认隐藏与明确显示、启用不自动监听、关闭两窗及监听、与按键迁移组合、幂等应用、反向开关、非法／重复输入，以及真实导入界面。本轮未打包或执行完整 GUI／游戏验证。

真实旧配置清点降至 25 条记录、20 个不同标识，见 `/tmp/voidmei-model-window-import-inventory.tsv` 和 `/tmp/voidmei-model-window-import-inventory.log`。该映射覆盖旧设置行；其他文件仅以面板属性保存总开关的格式仍需另行核对。下一批模型选择 selectedFM0／1 是离线对比机型标识，powerCurveSpeed 明确为 IAS 0–800 km/h，powerCurveWep 是功率图工况选择，仍未迁移。
