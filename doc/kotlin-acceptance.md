...
## 推力窗口可选延迟关闭（2026-09-12）

新增默认关闭的 `modelJetWindowAutoClose`。联动推力窗口有效且未启用模型热键时，已知有效起落架小于 100%，或 TAS > 36 km/h 且任一发动机有效油门 > 0，会触发 10 秒后隐藏本次推力窗口。首次触发后不因条件回落撤销；遥测中断会取消尚未完成的计时。计时使用协程时钟，不积压遥测更新。

关闭状态仅属于当前启用区间和模型会话，不写回显示偏好；切换机型／连接会话、重新开关模型浮窗、关闭此策略或启用模型热键可恢复。自动关闭只影响推力窗，不影响模型详情或遥测。缺失、越界及非有限条件不会误触发，速度使用明确 TAS 阈值；不复刻 Java 经平滑修正的 speedv 和模糊 throttle 字段查找。界面说明任一发动机油门判据及恢复方式。

共享 JVM 593、JS 590、桌面单元 153、相关 GUI 53 项通过，无失败、错误或跳过，日志 `/tmp/voidmei-jet-dismissal-tests.log` 和最终 `/tmp/voidmei-jet-dismissal-final.log`。覆盖阈值／无效输入、10 秒延迟与触发锁定、遥测中断取消、策略关闭和会话重置；真实窗口测试确认到期销毁、保存偏好保留及启用热键后恢复且不再自动关闭。本轮未打包或执行完整 GUI／真实游戏验收。

旧 `enableFMPrint` 尚未提供导入预览映射；所需的模型窗、联动推力窗、热键和可选延迟策略已有对应能力，但旧总开关的启用与初始隐藏意图还需明确呈现。真实配置差异仍为 26 条记录、21 个标识。
