...
2026-09-12，使用当前 KMP `LegacySettingsReader` 读取仓库自带 `ui_layout.cfg`。返回 **25 条未迁移记录、20 个不同标识（含 target 和面板属性名）**；`fontSize` 在多个面板出现。此数字表示该文件的配置导入差异，不能直接解释成 20 个未实现功能，也不覆盖用户自定义文件中的全部格式。
...
| 模型选择和曲线状态 | `selectedFM0`、`selectedFM1`、`powerCurveSpeed`、`powerCurveWep` | 已支持 KMP 离线目录及两个中央机型编号持久化；旧列表来自 FM 文件名，仍需解析到中央机型并处理歧义。功率速度与 WEP 输入迁移尚未接入。 |
...
本轮使用当前 `core/build/libs/core-jvm.jar`，并从已经构建的独立包提供 Kotlin 运行依赖执行工具，结果位于 `/tmp/voidmei-model-window-import-inventory.tsv`，统计日志 `/tmp/voidmei-model-window-import-inventory.log`。这些临时路径不作为仓库的永久数据源，生产代码或旧配置改变后应重新生成。
...
旧模型窗口总开关 `enableFMPrint` 设置行现可选择迁移：启用时准备推力联动和延迟策略，默认隐藏，可另选立即显示；关闭时同时关闭模型监听。位置、按键和分类保持。只含面板属性而无对应行的其他格式未据此宣称覆盖；旧图表的模糊油门和速度平滑没有复刻。
