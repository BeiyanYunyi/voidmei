...
| FM 模型信息浮窗 | `enableFMPrint`、`displayFmKey` | Java 在 `Controller` 注册独立模型数据浮窗，且同一开关控制喷气推力图；全局热键切换浮窗。Kotlin 已有独立当前模型浮窗，共享模型、燃油和分类选择，支持置顶与位置保存；尚未完成旧喷气推力图关联及模型热键，故本轮仍不直接迁移。新增热键需先整合现有 `NativeHotkeyBackend` 的独占钩子生命周期。 |
