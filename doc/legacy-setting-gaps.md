...
2026-09-12，使用当前 KMP `LegacySettingsReader` 读取仓库自带 `ui_layout.cfg`。返回 **36 条未迁移记录、31 个不同标识（含 target 和面板属性名）**；`fontSize` 在多个面板出现。此数字表示该文件的配置导入差异，不能直接解释成 31 个未实现功能，也不覆盖用户自定义文件中的全部格式。
...
| 模型分类显示 | `showMaxLiftLoad`、`showLift`、`showDrag`、`showNoFlapsWing`、`showFullFlapsWing`、`showFuselage`、`showFin`、`showStab` | 当前已有可保存的 13 类模型详情显示选择与原始字段筛选，实时和离线页面共用。重量、临界速度、过载、襟翼、舵效、转动惯量、加力信息和耐热恢复八类显示意图已支持可选迁移；本行其余 8 类尚未映射。襟翼数据点表与三舵 PowerLoss 原始系数（旧锁舵因数）已补齐，其余模型内容继续核对，不能从显示开关可迁移推导内容完全一致。逐项对照显示内容、单位、缺失数据和选择状态。 |
...
本轮使用当前 `core/build/libs/core-jvm.jar`，并从已经构建的独立包提供 Kotlin 运行依赖执行工具，结果位于 `/tmp/voidmei-thermal-recovery-inventory.tsv`，统计日志 `/tmp/voidmei-thermal-recovery-inventory.log`。这些临时路径不作为仓库的永久数据源，生产代码或旧配置改变后应重新生成。
