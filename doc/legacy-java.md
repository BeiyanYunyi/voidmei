...
本页保留旧版构建、使用及 Linux/Wine 运行说明，用于回退与迁移对照。当前默认入口为 Kotlin，见 [项目首页](../README.md)。本页命令均在仓库根目录执行。
...
本节仅用于旧版回退和维护。默认 Nix 入口现为 Kotlin，日常运行见上方 [Kotlin 版试用步骤](kotlin-quick-start.md)。
