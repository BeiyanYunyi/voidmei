    ...
    Column(Modifier.fillMaxSize().testTag("hud-panel")
        ...
        Column(Modifier.fillMaxWidth().padding(end = 8.dp).verticalScroll(bodyScroll)
            ...
            flight?.let {
                ...
                if (settings.hudAttitude) AttitudePanel(it.telemetry, compact = true, model = model, earthFixed = settings.hudAttitudeEarthFixed, showNorthPointer = settings.hudAttitudeNorthPointer, refreshMs = settings.hudAttitudeRefreshMs, showAoaLimits = settings.hudAttitudeAoaLimits)
            }
        }
