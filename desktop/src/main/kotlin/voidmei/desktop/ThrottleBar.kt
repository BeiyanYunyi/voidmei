...
internal fun ThrottleBar(flight: ConnectionState.Flying) {
    val throttle = HudField.ENGINE1_THROTTLE.value(flight) ?: return
    LinearProgressIndicator(
        progress = { (throttle / 110).coerceIn(0.0, 1.0).toFloat() },
        modifier = Modifier.fillMaxWidth().testTag("hud-throttle-bar").semantics {
            contentDescription = "1 号油门，满刻度 110%"
