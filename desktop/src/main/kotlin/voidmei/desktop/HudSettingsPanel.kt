...
internal fun HudSettingsPanel(settings: AppSettings, onChange: (AppSettings) -> Unit) {
    ...
    SettingsPages("HUD 布局",
        SettingsDestination("hud-regions", "调整分区位置与透明度", "布局预设、画布与各分区配置") {
            HudSceneSettings(settings, onChange)
        },
        SettingsDestination("hud-behavior", "显示与交互", "失焦隐藏、鼠标穿透与兼容显示") {
        ...
        SettingsDestination("hud-layout", "尺寸与排列", "HUD 宽度、读数列数与高度来源") {
            Text("高度读数来源")
        ...
        SettingsDestination("hud-font", "字体与配色", "文字大小、数字字体与读数颜色") {
            GlobalHudFontSizeSettings(settings.hudFontScale) { onChange(settings.copy(hudFontScale = it)) }
        ...
        SettingsDestination("hud-engine", "发动机读数", "发动机编号与显示字段") {
        ...
        SettingsDestination("hud-instruments", "仪表与辅助显示", "姿态、机械化、准星、罗盘及迎角预警") {
        ...
        SettingsDestination("hud-reset", "恢复 HUD 布局", "恢复默认配置或撤销本次重置") {
            TextButton(onClick = {
        ...
        SettingsDestination("hud-fields", "HUD 字段设置", "飞行读数字段、标签与显示顺序") {
