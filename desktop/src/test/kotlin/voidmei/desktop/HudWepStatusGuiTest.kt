...
class HudWepStatusGuiTest {
    ...
    @Test fun upperBoundsRemainWithoutHudExplanationsAcrossSamplingChanges() {
        ...
        compose.onAllNodesWithText("WEP 为估算上限", substring = true).assertCountEquals(0)
        ...
        compose.onNodeWithText("当前未观测到 WEP 消耗，续航时间未知。").assertDoesNotExist()
        compose.runOnIdle { flight = sample(null, 3000) }
        compose.onNodeWithText("暂无有效 WEP 估算", substring = true).assertDoesNotExist()
        ...
        compose.onNodeWithText("不表示实际补满", substring = true).assertDoesNotExist()
